...
cd setup-mpc-client
docker build -t aztecprotocol/setup-mpc-client .
cd ../
curl -X POST -i -H $CT -H $SIG -c cookies.txt -X POST http://localhost:8081/api/reset -d "$(cat <<EOF
{
	"startTime": 10,
	"endTime": 60,
	"selectBlock": -1,
